...
import { TimescaleAdapter, InMemoryAdapter } from '../../../../apps/web/src/lib/monitoring/persistence/timescale-adapter';
...
jest.mock('../../../../apps/web/src/lib/monitoring/config/realtime-config', () => ({
  realtimeConfig: {
    storage: {
      batchSize: 100,
      maxRetries: 3,
...
describe('TimescaleAdapter', () => {
  ...
  describe('Batch Processing - Stack Overflow Fix', () => {
    ...
    it('should handle continuous batch additions during processing', async () => {
      ...
      const originalProcessBatch = (adapter as unknown as { processBatch: (batch: unknown) => Promise<void> }).processBatch;
      (adapter as unknown as { processBatch: jest.Mock }).processBatch = jest.fn(async (_batch) => {
        processedBatches++;
        // Small delay to simulate processing time
        await new Promise(resolve => setTimeout(resolve, 1));
        return originalProcessBatch.call(adapter, _batch);
      });
    ...
    it('should handle batch processing errors without infinite recursion', async () => {
      ...
      const originalInsertMetrics = (adapter as unknown as { insertMetrics: (metrics: unknown) => Promise<void> }).insertMetrics;
      (adapter as unknown as { insertMetrics: jest.Mock }).insertMetrics = jest.fn(async (_metrics) => {
        if (failCount < maxFails) {
          failCount++;
          throw new Error(`Simulated failure ${failCount}`);
        }
        return originalInsertMetrics.call(adapter, _metrics);
      });
      ...
      expect((adapter as unknown as { insertMetrics: jest.Mock }).insertMetrics).toHaveBeenCalled();
    });
    ...
    it('should respect processing flag to prevent concurrent processing', async () => {
      ...
      const originalProcessPendingBatches = (adapter as unknown as { processPendingBatches: () => Promise<void> }).processPendingBatches;
      (adapter as unknown as { processPendingBatches: jest.Mock }).processPendingBatches = jest.fn(async function(_this: TimescaleAdapter) {
        concurrentCalls++;
