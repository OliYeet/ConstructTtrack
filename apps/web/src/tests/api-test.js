...
class ApiTester {
  ...
  async run() {
    ...
    await this.test('Health Check Endpoint', async () => {
      ...
      console.log(
        `   Services: API=${data.data.services.api}, DB=${data.data.services.database}`
      );
    });
    ...
    await this.test('Rate Limiting', async () => {
      ...
      if (!allSuccessful) {
        console.log(
          '   Some requests were rate limited (this is expected behavior)'
        );
      } else {
    ...
    await this.test('CORS Headers', async () => {
      ...
      if (response.status !== 200) {
        throw new Error(
          `Expected status 200 for OPTIONS but got ${response.status}`
        );
      }
    ...
    console.log(
      `📈 Success Rate: ${Math.round((this.passed / (this.passed + this.failed)) * 100)}%`
    );
