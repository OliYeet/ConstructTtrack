          ...
          case 'details': {
            ...
            if (!fingerprint) {
              return createErrorResponse(
                new BaseApiError(
                  'Fingerprint parameter required for details view',
                  400,
                  'VALIDATION_ERROR'
                ),
                'unknown'
            ...
            const errorDetails = errorReporter
              .getAggregatedErrors()
            ...
            response = {
              error: errorDetails,
              relatedErrors: errorReporter
                .getAggregatedErrors()
                .filter(
                  e =>
                    e.classification.type === errorDetails.classification.type
                )
                .slice(0, 5),
            };
            break;
...
export const POST = withAuth({
  POST: async (request: NextRequest) => {
    try {
      ...
      const { title, description, error, context, classification } = body;

      // Validate required fields
      if (!title || !description || !error) {
        return createErrorResponse(
          new BaseApiError(
            'Missing required fields: title, description, error',
            400,
            'VALIDATION_ERROR'
          ),
          'unknown'
    ...
    PATCH: async (request: NextRequest) => {
      try {
        ...
        if (!fingerprint) {
          return createErrorResponse(
            new BaseApiError(
              'Fingerprint required in request body',
              400,
              'VALIDATION_ERROR'
            ),
            'unknown'
