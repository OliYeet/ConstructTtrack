...
class NotionDatabaseSync {
    ...
    async start(port = 3001) {
        console.log(chalk.blue('🔄 Starting ConstructTrack Notion Sync Service...'));
        ...
        this.app.listen(port, () => {
            ...
            console.log(chalk.blue(`📡 Webhook endpoint: http://localhost:${port}/webhook`));
            console.log(chalk.yellow('💡 Configure this URL in your Notion integration settings'));
        });
    }
    ...
    setupWebhookEndpoint() {
        this.app.post('/webhook', async (req, res) => {
            try {
                ...
                if (false && this.webhookSecret && !this.verifyWebhookSignature(req)) {
                    console.log(chalk.red('❌ Invalid webhook signature'));
                    console.log(chalk.yellow('🔍 Debug: Webhook received but signature invalid'));
     ...
     */
    async handlePageChange(pageData) {
        if (this.syncInProgress) {
            console.log(chalk.yellow('⏳ Sync already in progress, queuing update...'));
            return;
        }
     ...
     */
    async handleDatabaseChange(databaseData) {
        console.log(chalk.blue('🗄️ Database structure changed, updating local schema...'));
    ...
    async syncNotionToLocal() {
        ...
        try {
            ...
            // Group by type
            const epics = response.results.filter(page =>
                page.properties.Type?.select?.name === 'Epic'
            );
            const stories = response.results.filter(page =>
                page.properties.Type?.select?.name === 'Story'
            );
            const tasks = response.results.filter(page =>
                page.properties.Type?.select?.name === 'Task'
            );

            // Generate updated markdown
            const markdown = await this.generateMarkdownFromNotion(epics, stories, tasks);

            // Write to file with backup
            this.createBackup('docs/constructtrack_agile_project_plan.md');
            ...
            console.log(chalk.green(`✅ Updated local files with ${response.results.length} items`));
    ...
    async syncLocalToNotion(filePath) {
        ...
        try {
            ...
            if (filePath.endsWith('.md')) {
                ...
                console.log(chalk.yellow('⚠️  Markdown sync temporarily disabled to prevent duplicates'));
        ...
        try {
            ...
            for (const block of response.results) {
                if (block.type === 'heading_1' || block.type === 'heading_2' || block.type === 'heading_3') {
                    ...
                    if (text.trim()) {
                        if (currentSection.includes('acceptance criteria') || text.includes('Acceptance Criteria')) {
                            content.acceptanceCriteria = text;
                        } else if (currentSection.includes('description') || text.includes('As a')) {
            ...
            console.log(chalk.gray(`Could not fetch content for page ${pageId}: ${error.message}`));
            return {};
        }
    }
    ...
    calculateProgress(items) {
        ...
        const completed = items.filter(item => {
            ...
            return status === 'Done' || status === 'Completed' || status === 'Deployed';
        }).length;
    ...
    async generateMarkdownFromNotion(epics, stories, tasks) {
        ...
        epics.forEach(epic => {
            const epicTitle = epic.properties.Name?.title?.[0]?.text?.content || 'Untitled Epic';
            epicMap.set(epic.id, {
                title: epicTitle,
        ...
        stories.forEach(story => {
            ...
            if (parentEpicId && epicMap.has(parentEpicId)) {
                const storyTitle = story.properties.Name?.title?.[0]?.text?.content || 'Untitled Story';
        ...
        tasks.forEach(task => {
            ...
            epicMap.forEach(epic => {
                ...
                if (story) {
                    const taskTitle = task.properties.Name?.title?.[0]?.text?.content || 'Untitled Task';
                    story.tasks.push({
                        id: task.id,
                        title: taskTitle,
        ...
        for (const [, epic] of epicMap.entries()) {
            ...
            for (const story of epic.stories) {
                ...
                // Story metadata
                if (storyPoints || storyContent.acceptanceCriteria || storyContent.dependencies) {
                    markdown += `**Story Details:**\n`;
                    if (storyPoints) markdown += `- **Story Points**: ${storyPoints}\n`;
                    if (storyContent.acceptanceCriteria) {
                        markdown += `- **Acceptance Criteria**:\n`;
                        const criteria = storyContent.acceptanceCriteria.split('\n').filter(line => line.trim());
                ...
                if (story.tasks.length > 0) {
                    ...
                    for (const task of story.tasks) {
                        ...
                        const role = task.properties['Team/Role']?.rich_text?.[0]?.text?.content || 'Team';
                        ...
                        if (taskContent.description) {
                            const taskDetails = taskContent.description.split('\n').filter(line => line.trim());
                            description = taskDetails
                                .filter(detail => !detail.includes('Story Points:') && !detail.includes('Role:'))
    ...
    async generateStatusFiles(allItems) {
        ...
        const statusData = {
            ...
            byType: {
                epics: allItems.filter(item => item.properties.Type?.select?.name === 'Epic').length,
                stories: allItems.filter(item => item.properties.Type?.select?.name === 'Story').length,
                tasks: allItems.filter(item => item.properties.Type?.select?.name === 'Task').length
            },
            byStatus: {}
        };
        ...
        writeFileSync('docs/project-status.json', JSON.stringify(statusData, null, 2));
        ...
        writeFileSync('docs/team-assignments.json', JSON.stringify(teamData, null, 2));
    }
    ...
    cleanupOldBackups(filePath) {
        try {
            ...
            console.log(chalk.gray(`Warning: Could not clean up old backups: ${error.message}`));
        }
    }
