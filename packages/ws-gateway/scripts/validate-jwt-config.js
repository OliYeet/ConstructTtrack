...
function validateEnvironmentVariables() {
  ...
  if (jwtSecret.length < 32) {
    logWarning(`JWT_SECRET is only ${jwtSecret.length} characters long. Recommended minimum is 32 characters.`);
  } else {
    logSuccess(`JWT_SECRET is properly configured (${jwtSecret.length} characters)`);
  }
...
function testClockSkew() {
  ...
  try {
    ...
    logInfo('This may indicate clock synchronization issues in your environment');
    return false;
  }
}
...
function main() {
  ...
  if (allTestsPassed) {
    ...
    logError('Some JWT configuration tests failed. Please review the output above.');
    process.exit(1);
  }
}
