...
const CONFIG = {
  ...
  previousSpecPath: process.argv[2] || path.resolve(__dirname, '../docs/api/openapi.yaml.backup'),
...
async function generateOpenApiDiff() {
  ...
  try {
    ...
    if (!fs.existsSync(CONFIG.previousSpecPath)) {
      console.log(chalk.yellow('⚠️  Previous spec file not found. Creating backup of current spec...'));
      await createBackup();
      console.log(chalk.green('✅ Backup created. Run diff again after making changes.'));
      return;
    }
...
async function saveDiffReport(diffResult) {
  try {
    ...
    if (CONFIG.format === 'json') {
      // Save JSON report
      const jsonReportPath = path.join(CONFIG.outputDir, `openapi-diff-${timestamp}.json`);
      ...
      const mdReportPath = path.join(CONFIG.outputDir, `openapi-diff-${timestamp}.md`);
      fs.writeFileSync(mdReportPath, markdownReport);
      console.log(chalk.green(`📊 Markdown diff report saved: ${mdReportPath}`));
    }
    
  } catch (error) {
    console.warn(chalk.yellow(`⚠️  Failed to save diff report: ${error.message}`));
  }
}
...
function displayDiffResults(diffResult) {
  ...
  console.log(`${chalk.gray('⚪')} Unclassified Changes: ${unclassifiedChanges.length}`);
  
  if (breakingChanges.length > 0) {
    console.log(chalk.red.bold('\n⚠️  BREAKING CHANGES DETECTED!'));
    console.log(chalk.red('This may require a major version bump and migration guide.'));
    
    if (CONFIG.verbose) {
      console.log(chalk.red.bold('\nBreaking Changes:'));
      breakingChanges.forEach((change, index) => {
        console.log(chalk.red(`  ${index + 1}. ${change.type || 'Change'} at ${change.location || 'unknown location'}`));
        console.log(chalk.red(`     ${change.description || change.message || 'No description'}`));
      });
    }
  } else if (nonBreakingChanges.length > 0) {
  ...
  if (breakingChanges.length > 0) {
    ...
    console.log(chalk.green('  Patch version bump (e.g., v1.0.0 → v1.0.1) if needed'));
  }
}
