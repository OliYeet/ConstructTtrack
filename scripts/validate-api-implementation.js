...
async function validateApiImplementation() {
  ...
  try {
    ...
    console.error(
      chalk.red.bold('❌ Implementation validation failed:'),
      error.message
    );
    if (CONFIG.verbose) {
...
function prepareRequest(pathTemplate, method, operationSpec, api) {
  ...
  url = url.replace(/\{([^}]+)\}/g, (_match, _paramName) => {
  ...
  if (['post', 'put', 'patch'].includes(method) && operationSpec.requestBody) {
    options.body = JSON.stringify(
      generateTestRequestBody(operationSpec.requestBody, api)
    );
  }
 ...
 */
function generateTestRequestBody(requestBodySpec, _api) {
 ...
 */
async function validateResponse(testCase, operationSpec, _api) {
...
async function generateTestReport() {
  try {
    ...
    const reportPath = path.join(
      CONFIG.outputDir,
      `implementation-test-report-${Date.now()}.json`
    );
